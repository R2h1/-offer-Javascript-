 ...
 *          每轮判断滑动窗口内元素和与目标值 target 的大小关系，若相等则记录结果，若大于 target 则移动左边界 i （以减小窗口内的元素和），若小于 targe 则移动右边界 j （以增大窗口内的元素和）。
 */
    ...
    for (let i = 1; i < limit; i++) {
        ...
        while (true) {
            ...
            if (sum > target) {
                ...
                res.push(new Array(j - i + 1).fill(i).map((item, index) => {
                    return item + index;
                }))
                break;
            }
...
function findContinuousSequence3(target: number): number[][] {
    ...
    while (left < right) {
        if (sum === target) {
            res.push(new Array(right - left + 1).fill(left).map((item, index) => {
                return item + index;
            }))
        } 
        if (sum >= target) {
