...
function isNumber3(s: string): boolean {
    ...
    while (left <= right) {
        const char = s[left];
        if (char >= '0' && char <= '9') { // 当前是数字
            isNum = true;
        } else if (['e', 'E'].includes(char)) {
            if (hasE || !isNum) { // 之前必须是数值 且不能有 e | E
                return false
            }
            hasE = true;
            isNum = false; // 包括 e | E 在内的已经不是数值，若继续遍历下一个是（+ | -）数字，则才能是数值，所以需要置为false
        } else if (char === '.') { // 遇到 . 的时候不需要改变 isNum
            if (hasDot || hasE) { // 小数点前面不能有小数点和 e | E
                return false
            }
            hasDot = true;
        } else if (['+', '-'].includes(char)) { // 只能出现在首位和 e 后面的首位置
