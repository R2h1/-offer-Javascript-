...
function GetNext(node: TreeLinkNode | null) {
    ...
    if (node.right !== null) { // 该节点有右子树，下一节点为右子树中的最左节点
        let nextNode = node.right;
        while (nextNode.left !== null) { // 向左遍历直到左子树为空
            nextNode = nextNode.left;
        }
        return nextNode;
    }
    // 该节点没有右子树
    while (node.next !== null) { // 向上不断找父节点
        if (node.next.left === node) { // 直到当前节点的父节点的左孩子是该节点，返回当前节点父节点
            return node.next;
        }
        node = node.next
    }
