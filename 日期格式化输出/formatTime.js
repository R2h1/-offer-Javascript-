Date.prototype.format = function(fmt) {
	...
	if (/(w+)/.test(fmt)) {
		fmt = fmt.replace(RegExp.$1, (RegExp.$1.length > 2 ? "星期" : "周") + ['日', '一', '二', '三', '四', '五', '六'][this.getDay()]);
	}
	if (/(q+)/.test(fmt)) {
		fmt = fmt.replace(RegExp.$1, (RegExp.$1.length > 3 ? "第" : "") + ['一', '二', '三', '四'][Math.floor((this.getMonth() + 3) / 3)] + "季度");
	}
	for (let k in o) {
