class Heap {
    ...
    insert(data: number) {
        ...
        while (index) { // 直到遍历到堆顶
            ...
            if (!this.cmp(this.container[index], this.container[parent])) { // 大顶堆：当前节点不大于父节点，到达最终位置
                return;
            }
            // 交换
    ...
    delete(): number {
        ...
        while (exchange < size) {
            // 右子节点位置
            let right = index * 2 + 2;
            if (right < length && this.cmp(this.container[right], this.container[exchange])) { // 大顶堆：存在右节点且右节点较大
                exchange = right;
            }
            if (!this.cmp(this.container[exchange], this.container[index])) {   // 大顶堆：子节点较大者小于当前节点
                return NaN;
            }
            // 交换
