...
function spiralOrder1(matrix: number[][]): number[] {
    ...
    while (layer * 2 < row && layer * 2 < col) { // 每遍历一次，行和列均减去了2，
        const endRowIndex = row - 1 - layer;
        const endColIndex = col - 1 - layer;
        for (let j = layer; j <= endColIndex; j++) {  // 从左向右
            printResult.push(matrix[layer][j]);
        }
        for (let i = layer + 1; i <= endRowIndex; i++) { // 从上到下 
            printResult.push(matrix[i][endColIndex]);
        }
        if (layer < endRowIndex) {  // 排除只剩下一行，不需要从右到左
            for (let i = endColIndex - 1; i >= layer; i--) { // 从右到左
                printResult.push(matrix[endRowIndex][i]);
            }
        }
        if (layer < endColIndex) {  // 排除只剩下一列，不需要从下到上
            for (let j = endRowIndex - 1; j > layer; j--) { // 从下到上
