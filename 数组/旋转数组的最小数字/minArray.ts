...
function minArray2(numbers: number[]): number {
    ...
    while (left < right) {
        ...
        if (midNum > rightNum) { // 中间数大于右边数， 最小元素在[mid + 1, right]
            left = mid + 1;
        } else if (midNum < rightNum) { // 中间数小于右边数， 最小元素在[left, mid]
            right = mid;
        } else { // 中间的数等于右边，而且左边大于等于右边 left >= mid = right, 最小元素在[left + 1, right - 1]
            left = left + 1;
            right = right - 1;
        }
    }
