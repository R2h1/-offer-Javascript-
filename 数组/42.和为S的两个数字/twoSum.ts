...
function twoSum1(nums: number[], target: number): number[] {
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (hash.has(sub)) { // 可以使用 set.has(sub) 或  hash[sub] !== undefined (考虑到差值是0)
...
function twoSum3(nums: number[], target: number): number[] {
    ...
    while (left < right) {
        ...
        if (sum === target) {
            ...
            break;
        } else if (sum < target) { // 小于目标值，说明需要增加加数的值，移动左指针
            left = left + 1;
        } else { // 大于目标值，说明需要减少加数的值，移动右指针
            right = right - 1;
        }
...
function twoSum4(nums: number[], target: number): number[] {
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (binarySearch(i + 1, len - 1, sub) !== -1) {  // 在当前元素右侧寻找即可
            res.push(element, sub);
            return res;
        }
    }
...
function twoSum5(nums: number[], target: number): number[] {
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (![-1, i].includes(indexOfSub)) { // 存在且不等于当前下标
            res.push(element, sub);
            return res;
        }
    }
